...
页面默认目标已经内置 Cloudflare 官方 IPv4 段每段 1 个代表 IP，共 15 个。来源是 Cloudflare 官方 IPv4 列表：`https://www.cloudflare.com/ips-v4`。其中大部分样本已实测能返回 `cf-ray`；少数官方段如果不响应 HTTP，结果里会显示超时或失败。
